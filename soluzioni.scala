...
def solveEquation( a: Int, b: Int, c: Int ): Unit = {
  ...
  val delta = scala.math.pow(b, 2) - 4 * a * c
    if (delta < 0) {
        ...
        val x = -b / (2 * a)
        println("One solution: " + x)
    } else {
        val x1 = (-b + scala.math.sqrt(delta)) / (2 * a)
        val x2 = (-b - scala.math.sqrt(delta)) / (2 * a)
        println("Two solutions: " + x1 + " and " + x2)
    }
}

//stampa tutti i caratteri della stringa in input
def printAllChars( string: String ): Unit = {
    println("All chars of the string: " + string)
    for (i <- 0 until string.length) {
        println(string.charAt(i))
    }
}

//Scrivere un algoritmo che preso in input un numero X a 5 cifre,
//visualizza in ordine decrescente i numeri naturali da X a 150 a step di 2. 
//Stampare “ERRORE” in caso il numero inserito non rispetta i requisiti e ripetere l’inserimento
